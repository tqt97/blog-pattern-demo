            ...
            <div class="mt-4 sm:-mx-6 sm:px-6 lg:-mx-8">
                <div
                    ...
                    <div class="flex flex-col gap-1 lg:flex-row lg:items-center lg:justify-between">
                        ...
                        <x-bulk-actions action="{{ route('admin.posts.bulk') }}" :trashed="$trashed" :normalActions="$postNormalBulkActions"
                            :trashActions="$postTrashBulkActions" />
                    ...
                    <x-filters :action="route('admin.posts.index')" :baseExcept="['page']" class="flex items-center">
                        <div class="flex flex-wrap items-center gap-1 pt-4 border-t border-dashed border-slate-200">
                            ...
                            <x-filters.select-field name="category_id" :options="$categories" placeholder="Select category"
                                :autoSubmit="true" />

                            <x-filters.select-field name="tag_id" :options="$tags" placeholder="Select tag"
                                :autoSubmit="true" />
                            <x-filters.select-field name="status" :options="$status" placeholder="Select status"
                                :autoSubmit="true" />
                                ...
                                @forelse ($posts as $post)
                                    <x-table.tr :isHighlight="$highlightId === $post->id">
                                        <x-table.td class="px-4 py-3">
                                            <input type="checkbox" value="{{ $post->id }}" x-model="selectedIds"
                                                ...
                                                <div class="flex flex-col">
                                                    <span class="font-semibold text-md">
                                                        <a href="{{ route('posts.show', $post) }}"
                                                            ...
                                                            {{ $post->short_title }}
                                                            <x-icons.arrow-top-right-on-square size="3" />
                                                        </a>
                                                    </span>
                                                    <div class="flex items-center gap-2">
                                        ...
                                        <x-table.td>
                                            ...
                                            <div class="text-right flex items-center justify-end gap-1">
                                                ...
                                                <x-table.row-actions :model="$post" baseRoute="admin.posts"
                                                    :trashed="request('trashed')" :usesSoftDeletes="true" :showView="false"
