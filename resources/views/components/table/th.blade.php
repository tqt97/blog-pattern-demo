...
@php
    ...
    $class = 'px-4 py-3 text-left text-sm font-semibold text-gray-100 uppercase1 tracking-wider';
@endphp
        ...
        <a href="{{ request()->fullUrlWithQuery(['sort' => $field, 'direction' => $nextDirection]) }}"
            ...
            @if ($isCurrent)
                @if ($currentDirection === 'asc')
                    <svg xmlns="http://www.w3.org/2000/svg" class="size-4 text-gray-100" viewBox="0 0 20 20"
                        fill="currentColor">
                ...
                @endif
            @else
                <svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3 text-gray-300 group-hover:text-gray-400"
